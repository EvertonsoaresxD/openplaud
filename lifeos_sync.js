...
// Configurações
const DB_URL = process.env.DATABASE_URL || 'postgresql://postgres:postgres@localhost:5432/openplaud';
const VAULT_PATH = 'D:\\Modelos Obsidian\\Vaults\\LifeOS-vault\\5-Expresso\\Inbox';
...
async function formatMarkdown(recording, transcription, enhancement) {
    ...
    const time = new Date(recording.start_time).toISOString().split('T')[1].substring(0, 5);
...
async function run() {
    ...
    try {
        console.log(`Buscando transcrições criadas após: ${syncTime.toISOString()}`);
        ...
        console.log(`Encontradas ${newTranscriptions.length} novas transcrições.`);
        ...
        for (const t of newTranscriptions) {
            ...
            `;
            const enhancement = enhancements.length > 0 ? enhancements[0] : null;

            const markdown = await formatMarkdown(t, t, enhancement);
            
            // Define o nome do arquivo, limpando caracteres inválidos
            const safeFileName = t.filename.replace(/[^a-zA-Z0-9 -_]/g, '').trim() || 'Sem_Nome';
            const uuid = t.id.substring(0, 5); // Adiciona um UUID curto caso hajam nomes duplicados
            const finalFilePath = path.join(VAULT_PATH, `${safeFileName}_${uuid}.md`);
        ...
        // Atualiza o estado
        fs.writeFileSync(STATE_FILE, JSON.stringify({ lastSync: new Date(latestTimestamp).getTime() }));
